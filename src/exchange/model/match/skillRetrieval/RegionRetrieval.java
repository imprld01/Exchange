...
public class RegionRetrieval implements SkillRetrievalSet {
	
	private int limit;
	private Area[] area;
	private String user_id;
	private int nowRegionNum;
	private ArrayList<CandidateSkill> existedSkill;
	
	public RegionRetrieval(Area [] area, String user_id, int limit, ArrayList<CandidateSkill> existedSkill){
	...
	public ArrayList<CandidateSkill> retrieveSkills(){
		...
		if(this.nowRegionNum < RealDistanceOrder.areaQuantity()){
			...
			for (int i = 0; i < this.existedSkill.size(); i++) {		
				sql = sql + "and skills.skill_id!='" + this.existedSkill.get(i).getSkill().getSkillId() + "' ";		
			}
