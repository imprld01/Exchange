...
public class Skill {
	...
	// 建構子(int)
	public Skill(int skillId) throws SQLException {
		...
		if (rs.next()) {
			...
			times = rs.getInt("times");
			score = new Score(rs.getInt("attitude_score"), rs.getInt("profession_score"), rs.getInt("teaching_score"),
					rs.getInt("frequency_score"), rs.getInt("satisfication_score"));
	...
	// 建構子() 用於新增技能
	public Skill(String userId, String intorExper, String typeName, ArrayList<String> image, ArrayList<String> vedio)
			throws SQLException {
		this.userId = userId;
